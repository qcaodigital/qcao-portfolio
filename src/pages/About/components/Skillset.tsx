...
export default function Skillset({ sectionRef }: SkillsetProps) {
	const [activeTech, setActiveTech] = useState<{ label: string; scrollAtClick: number } | null>(
		null
	);
					...
					{technologies.map(
						(tech: technology): JSX.Element => (
							<motion.li
								...
								data-is-active={activeTech?.label === tech.label ? 'true' : 'false'}
								data-faded={
									activeTech?.label !== tech.label && activeTech !== null
										? 'true'
										: 'false'
								}
