...
export default function App() {
	...
	const [direction, setDirection] = useWheelSwipe(['/', '/about', '/work']);

	return (
		<main className={styles.App} style={{ height: screenHeight }}>
			<div className={styles.background} />
			<p>{direction ? direction : 'null'}</p>
			<Nav
				setIsHBMOpen={setIsHBMOpen}
				isHBMOpen={isHBMOpen}
				exitDirection={direction}
				setExitDirection={setDirection}
			/>
				...
				{!isHBMOpen ? (
					<Switch location={location} key={location.pathname}>
						<Route
							...
							path='/'
							render={() => (
								<Home viewport={viewport} exitDirection={direction} setDirection={setDirection} />
							)}
						/>
						<Route exact path='/about' render={() => <About pathname={location.pathname} />} />
					</Switch>
				) : (
					<HBM key='hbm' navLinks={navLinks} isHBMOpen={isHBMOpen} setIsHBMOpen={setIsHBMOpen} />
				)}
			</AnimatePresence>
		</main>
	);
